-- Заполнение id по коду
CREATE OR REPLACE FUNCTION update_hold_part_place_id () 
RETURNS TRIGGER 
LANGUAGE plpgsql
AS $$
BEGIN 
    RAISE NOTICE 'update_hold_part_place_id begin';

    IF NEW.code IS NOT NULL AND NEW.space_id IS NULL THEN
        RAISE NOTICE 'update_hold_part_place_id get space_id';
        SELECT space_id
        INTO NEW.space_id
        FROM hold_part_id
        WHERE code = NEW.code AND ship_id = NEW.ship_id;
    END IF;

    IF NEW.left_bulkhead_code IS NOT NULL AND NEW.left_bulkhead_place_id IS NULL THEN
        RAISE NOTICE 'update_hold_part_place_id get left_bulkhead_place_id';
        SELECT space_id
        INTO NEW.left_bulkhead_place_id
        FROM hold_part_id 
        WHERE code = NEW.left_bulkhead_code AND ship_id = NEW.ship_id;
    END IF;

    IF NEW.right_bulkhead_code IS NOT NULL AND NEW.right_bulkhead_place_id IS NULL THEN
        RAISE NOTICE 'update_hold_part_place_id get right_bulkhead_place_id';
        SELECT space_id
        INTO NEW.right_bulkhead_place_id
        FROM hold_part_id
        WHERE code = NEW.right_bulkhead_code AND ship_id = NEW.ship_id;
    END IF;

    IF NEW.group_space_id IS NOT NULL AND NEW.group_id IS NULL THEN
        RAISE NOTICE 'update_hold_part_place_id get group_id';
        SELECT id
        INTO NEW.group_id
        FROM hold_group
        WHERE space_id = NEW.group_space_id AND ship_id = NEW.ship_id;
    END IF;

    RAISE NOTICE 'update_hold_part_place_id OK';

    RETURN NEW;
END;
$$;

CREATE OR REPLACE TRIGGER check_insert_update_hold_part
    BEFORE INSERT OR UPDATE
    ON hold_part
    FOR EACH ROW
    EXECUTE FUNCTION update_hold_part_place_id();

-- Заполнение code по space_id если его нет
CREATE OR REPLACE FUNCTION update_hold_part_id_code () 
RETURNS TRIGGER 
LANGUAGE plpgsql
AS $$
BEGIN 
    RAISE NOTICE 'update_hold_part_id_code begin';

    IF NEW.code IS NULL AND NEW.space_id IS NOT NULL THEN
        RAISE NOTICE 'update_hold_part_id_code get code';
        NEW.code = NEW.space_id;
    END IF;

    RAISE NOTICE 'update_hold_part_id_code OK';

    RETURN NEW;
END;
$$;

CREATE OR REPLACE TRIGGER check_insert_update_hold_part_id_code
    BEFORE INSERT OR UPDATE
    ON hold_part_id
    FOR EACH ROW
    EXECUTE FUNCTION update_hold_part_id_code();


-- Заполнение данных по грузам в частях трюма 

CREATE OR REPLACE FUNCTION update_hold_parts_level ()
AS $update_hold_parts_level$
DECLARE 
    result compartment%rowtype;
    matter_type cargo_category.matter_type%TYPE;
BEGIN 

    RAISE NOTICE 'update_hold_parts_level begin compartment:[%]', NEW;

    if NEW.density IS NULL THEN 
        RAISE NOTICE 'SKIP update_hold_parts_level, NEW.density IS NULL';
        RETURN NEW;
    END IF;

    FOR index in NEW.group_start_index..NEW.group_end_index LOOP

        RAISE NOTICE 'update_hold_parts_level hold_part:[%]', part;

        if NEW.level IS NOT NULL THEN 
            RAISE NOTICE 'update_hold_parts_level INSERT NEW.mass';  
            result = get_compartment_curve_level(NEW.ship_id, NEW.space_id, NEW.level);
        ELSE 
            RAISE NOTICE 'update_hold_parts_level INSERT no level';  
            result = get_compartment_curve_level(NEW.ship_id, NEW.space_id, 0);
        END IF;

        UPDATE 
            hold_part 
        SET 
            mass = result.volume*NEW.density,
            mass_shift_x = result.mass_shift_x,
            mass_shift_y = result.mass_shift_y,
            mass_shift_z = result.mass_shift_z
        WHERE 
            ship_id = NEW.ship_id AND group_index = index AND group_id = NEW.group_id;

        -- Check if new category is of bulk type and then update grain_moment for compartment entry;
        SELECT cc.matter_type INTO matter_type FROM cargo_category AS cc WHERE id = NEW.category_id;
        IF (matter_type = 'bulk') THEN
            UPDATE 
                hold_part 
            SET 
                grain_moment = get_grain_moment(NEW.ship_id, NEW.space_id, NEW.level)
            WHERE 
                ship_id = NEW.ship_id AND group_index = index AND group_id = NEW.group_id;
        END IF;

    END LOOP;

    RAISE NOTICE 'update_hold_parts_level OK';
END;
$update_hold_parts_level$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER check_update_hold_compartment_level
    AFTER INSERT OR UPDATE ON compartment
    FOR EACH ROW 
    EXECUTE FUNCTION update_hold_parts_level();



    
