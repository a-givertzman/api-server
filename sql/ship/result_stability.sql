...
CREATE TABLE if not exists result_stability (
  ...
  unit TEXT,
  comment TEXT,
  CONSTRAINT result_stability_check CHECK(char_length(title) <= 100),
  CONSTRAINT result_stability_pk PRIMARY KEY (id),
  CONSTRAINT result_stability_check CHECK(char_length(unit) <= 50),
  CONSTRAINT result_stability_check CHECK(char_length(comment) <= 1000),
);
