...
BEGIN 
    ...
    if NEW.density IS NULL THEN 
        RAISE NOTICE 'update_compartment INSERT NEW.density IS NULL';
        RETURN NEW;
    END IF;
...
-- Ищем что изменилось и пересчитываем остальное
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        END IF;
    ELSE 
        RAISE NOTICE 'update_compartment no TG_OP';
        RETURN NEW;
    END IF;
    ...
    RAISE NOTICE 'update_compartment end, NEW:[%]', NEW;
...
BEGIN 
    ...
    RAISE NOTICE 'get_compartment_curve_volume res level:[%]  volume:[%]', res.level, res.volume;
