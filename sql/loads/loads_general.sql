...
FROM (
    SELECT ship_id, SUM(mass) AS mass FROM (
        SELECT ship_id, mass FROM compartment WHERE loading_type='cargo' UNION ALL 
...
FULL OUTER JOIN (
    SELECT ship_id, SUM(mass) AS mass FROM (
        SELECT ship_id, mass FROM compartment WHERE loading_type='ballast' UNION ALL 
...
FULL OUTER JOIN (
    SELECT ship_id, SUM(mass) AS mass FROM (
        SELECT ship_id, mass FROM compartment WHERE loading_type='store' UNION ALL 
...
FULL OUTER JOIN (
    SELECT ship_id, SUM(mass) AS mass FROM (
        SELECT ship_id, mass FROM compartment WHERE loading_type='lightship' UNION ALL 
