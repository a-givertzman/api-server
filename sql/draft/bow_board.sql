...
CREATE TABLE if not exists bow_board (
  ...
  CONSTRAINT bow_board_name_rus_check CHECK(char_length(name_rus) > 0 AND char_length(name_rus) <= 50),
  CONSTRAINT bow_board_name_rus_unique UNIQUE (project_id, ship_id, name_rus, z),
