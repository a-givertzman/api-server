...
use crate::{api_server::ApiServer, config::Config, socket::message::message::Message};
///
/// 
...
impl TcpConnection {
    ...
    ///    - if read 0 bytes
    ///    - if on error
    fn read_buf(self_id: &str, mut stream: impl Read, buf: &mut [u8]) -> ConnectionStatus {
        ...
        loop {
            match stream.read(buf) {
                ...
                Err(err) => {
                    ...
                    return parse_socket_error(self_id, err);
                    // return ConnectionStatus::Closed;
                },
            };
        }
