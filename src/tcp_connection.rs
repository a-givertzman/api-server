...
use api_tools::{
    api::{
        message::{fields::{FieldData, FieldId, FieldKind, FieldSize, FieldSyn}, message::{Message, MessageField}, message_kind::MessageKind, parse_data::ParseData, parse_id::ParseId, parse_kind::ParseKind, parse_size::ParseSize, parse_syn::ParseSyn},
...
impl TcpConnection {
    ...
    //                         v if v.is_empty() => {}
    //                         [..] => {
    //                             let err = format!("{} | Unknown message kind {:?}", self.dbgid, parsed);
    //                             log::warn!("{}", err);
    //                             // return Err(err.into())
    //                         }
    //                     }
